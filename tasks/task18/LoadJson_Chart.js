...
async function drawCurrentAssessmentChart() {
  ...
  const options = {
    chart: {
      type: 'line', // can change to 'bar' if needed
      height: 450,
      toolbar: { show: false }
    },
    stroke: {
    ...
    title: {
      text: 'Prior-Year Assessment Values',
      align: 'center'
    }
  };

  const chart = new ApexCharts(document.querySelector("#chart-prior-values"), options);
  chart.render();
}

document.addEventListener('DOMContentLoaded', drawCurrentAssessmentChart);
